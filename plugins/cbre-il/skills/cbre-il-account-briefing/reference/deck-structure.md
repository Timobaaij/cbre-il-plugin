...
- `supply-chain-signature` - what is distinctive about THIS company's supply chain, how it shapes the set-up, and how CBRE should approach it. See `reference/supply-chain-signatures.md`.
...
Open the deck on the executive read (`bluf`) so the reader gets the thesis, the big movers and the one opportunity before any detail. Open each chapter by explaining its argument in prose, then place the evidence: a `table` for a list or register, a `stat` row for the hero numbers, a `panel` for an at-a-glance lens. Open the supply-chain act on the **supply-chain signature** (`reference/supply-chain-signatures.md`): the two to four product and demand characteristics that actually dictate this network's shape, the set-up consequence of each, and how CBRE should approach it, before the operating model and the footprint explain how it runs and where it sits. Tell the big events (an acquisition, divestiture, JV, large committed capex) as dedicated prose-led scenes that say what happened, why, and what it means, never a single table row, and bring them together in the `big-movers` read so the reader sees the handful of vectors at once. Length follows the story; G0 judges whether the allocation fits it.
